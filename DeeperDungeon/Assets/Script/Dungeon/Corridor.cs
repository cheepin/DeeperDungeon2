...
namespace dungeon
{
	...
	public class Corridor
	{
		...
		Direction ChangeCorridorDirectionIfBeyondBoard(Direction direct,Room room,int maxCorridorLength,int maxRoomWidth,int maxRoomHeight,int columns,int rows,int callCount=0)
		{
			direct = (Direction)((int)direct % 4);
			int threshold = 8;
			if(callCount>6){
			...
			switch(direct)
			{
				case Direction.North:
					if(room.yPos + room.roomHeight+ maxCorridorLength + maxRoomHeight > columns-threshold)
					{
						direct = ChangeCorridorDirectionIfBeyondBoard(++direct,room, maxCorridorLength, maxRoomWidth, maxRoomHeight, columns, rows,++callCount);
					}
					break;

				case Direction.South:
					if(room.yPos - maxCorridorLength - maxRoomHeight < threshold)
					{
						direct = ChangeCorridorDirectionIfBeyondBoard(++direct, room, maxCorridorLength, maxRoomWidth, maxRoomHeight, columns, rows,++callCount);
					}
					break;
				...
				case Direction.East:
					if(room.xPos + room.roomWidth + maxCorridorLength + maxRoomWidth > rows - threshold)
					{
						direct = ChangeCorridorDirectionIfBeyondBoard(++direct, room, maxCorridorLength, maxRoomWidth, maxRoomHeight, columns, rows, ++callCount);
					}
					break;
				...
				case Direction.West:
					if(room.xPos - maxCorridorLength - maxRoomWidth < threshold)
					{
						direct = ChangeCorridorDirectionIfBeyondBoard(++direct, room, maxCorridorLength, maxRoomWidth, maxRoomHeight, columns, rows,++callCount);
					}
					break;
		...
		public void SetupCorridor(Room room, IntRange length, IntRange roomWidth, IntRange roomHeight, int columns, int rows,int maxRoomWidth,int maxRoomHeight, bool firstCorridor)
		{
			...
			direction = ChangeCorridorDirectionIfBeyondBoard(direction,room,length.m_Max,maxRoomWidth,maxRoomHeight,columns,rows);
			// Set a random length.
